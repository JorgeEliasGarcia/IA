...
void Arbol::AnadirNodoAbierto(const Nodo& nodo) {
  ...
  for(unsigned i{0}; i < abiertos_.size(); ++i) {
    if(abiertos_[i].i == nodo.i && abiertos_[i].j == nodo.j) { //Se trata del mismo nodo
     if(abiertos_[i].f <= nodo.f) { //El nodo ya estaba en abiertos con una f(n) menor. No lo añadimos a abiertos pero sí a cerrados
      cerrados_.push_back(nodo); 
     } else {
         cerrados_.push_back(abiertos_[i]); 
